...
namespace clayPay
{
  ...
  export class NewTransaction implements INewTransaction
  {
    ...
    public Validate():boolean
    {
      ...
      let payer = clayPay.CurrentTransaction.TransactionCashierData.ValidatePayer();
      if (!payer)
      {
    ...
    UpdateForm()
    {
      ...
      let amount = clayPay.CurrentTransaction.TotalAmountRemaining.toFixed(2);
      if (!clayPay.CurrentTransaction.CCData.Validated) Utilities.Set_Value(CCPayment.AmountPaidInput, amount);
      if (!clayPay.CurrentTransaction.CheckPayment.Validated) Utilities.Set_Value(Payment.checkAmountInput, amount);
      if (!clayPay.CurrentTransaction.CashPayment.Validated) Utilities.Set_Value(Payment.cashAmountInput, amount);
    }
